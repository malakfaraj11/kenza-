...
import { MessageCircle, QrCode, RefreshCw, CheckCircle2, ShieldCheck, Loader2, Power, Clock, Save, Sparkles, Activity, Send } from 'lucide-react';
...
export function WhatsAppConnectionView() {
  ...
  const [trackingData, setTrackingData] = useState<{
    ...
    trackedClients: Array<{ jobId: string; phone: string; totalDelayMinutes: number; remainingSeconds: number; createdAt: string }>;
    recentRelances: Array<{ id: number; telephone: string; message_relance: string; canal: string; statut: string; cree_le: string }>;
  }>({ activeTrackingCount: 0, trackedClients: [], recentRelances: [] });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-6 space-y-5 sm:space-y-6">
        ...
        {/* LIVE TRACKING ACTIVITY & RECENT FOLLOW-UPS */}
        <div className="border-t border-gray-100 pt-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Activity className="w-4 h-4 text-indigo-600 animate-pulse" />
              <h4 className="font-bold text-xs sm:text-sm text-gray-900">
                Surveillance du Tracking en Temps Réel
              </h4>
            </div>
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100">
              {trackingData.activeTrackingCount} client(s) sous surveillance
            </span>
          </div>

          {trackingData.activeTrackingCount > 0 ? (
            <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 space-y-2">
              <p className="text-xs font-semibold text-amber-900 flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5 text-amber-600" />
                Clients inactifs détectés (en attente de relance automatique) :
              </p>
              <div className="space-y-1.5">
                {trackingData.trackedClients.map((c) => (
                  <div key={c.jobId} className="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-amber-200/60 text-xs shadow-xs">
                    <span className="font-mono font-medium text-gray-800">
                      📱 {c.phone}
                    </span>
                    <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 font-semibold text-[11px]">
                      ⏱️ Relance dans ~{c.remainingSeconds}s
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-3 text-xs text-emerald-800 flex items-center gap-2">
              ...
              <span>Aucun client inactif en attente. Toutes les conversations sont à jour ou ont été finalisées en commande !</span>
            </div>
          )}

          {/* HISTORIQUE DES DERNIÈRES RELANCES */}
          {trackingData.recentRelances && trackingData.recentRelances.length > 0 && (
            <div className="space-y-2 pt-2">
              <h5 className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
                <Send className="w-3.5 h-3.5 text-indigo-600" />
                Derniers messages de relance automatique envoyés :
              </h5>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {trackingData.recentRelances.map((rel) => (
                  <div key={rel.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-1">
                    <div className="flex items-center justify-between text-[11px] text-gray-500">
                      <span className="font-bold text-gray-700">Client: {rel.telephone}</span>
                      <span>{new Date(rel.cree_le).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <p className="text-gray-800 italic">"{rel.message_relance}"</p>
                  </div>
                ))}
              </div>
            </div>
